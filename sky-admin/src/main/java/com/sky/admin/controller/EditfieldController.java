...
public class EditfieldController {
    ...
    private final String V = "editField";
    ...
    public String goUpdate(Map<String, Object> map,@PathVariable String id){
        ...
        map.put("ef",fieldDao.getEditField(id));
        map.put("id",id);
        return V+"/edit";
    }
    /**
    ...
    public List<EditField> goData(@PathVariable String id){
        List<EditField> efs=configurationPageService.getEditFields(id);
        return efs;
    }
    /**
     * 更新数据
    ...
    @ResponseBody
    public Boolean update(Map<String, Object> map,EditField editField,@PathVariable String id){
        editField.setCoding(id);
        editField.setInputCoding(editField.getInputCoding().equals("") ? null : editField.getInputCoding());
        if(fieldDao.doUpdateEditField(editField)){
